...
# --- Депозит за разовый урок (анти-неявка) ---
# Каждый новый ученик получает один бесплатный пробный урок на всю платформу.
# После того как пробный использован (посещён ИЛИ пропущен — неявка), любая
# последующая разовая бронь требует депозита на эту сумму. Депозит — не доп.
# платёж: он и есть оплата разового урока (после completed уходит учителю минус
# комиссия). При неявке ученика депозит сгорает; при вине учителя/несостоявшемся
# уроке — возвращается. Не применяется к подписочным урокам (они предоплачены).
# Сумма настраивается здесь/через env, нигде не хардкодится (см. billing.deposits).
BOOKING_DEPOSIT_AMOUNT = Decimal(os.environ.get('BOOKING_DEPOSIT_AMOUNT', '30000'))
